...
The project is designed as a production-style engineering portfolio: it includes role-based access control, secure token rotation, background processing, observability, automated tests, container builds, and CI/CD workflows.
...
- **Production tooling:** Docker Compose, GitHub Actions, Android release automation, Prometheus, Grafana, OpenTelemetry/Jaeger, Sentry integration points, and environment-injected secrets.
...
Docker Compose runs PostgreSQL/PostGIS, Redis, the API, worker, Prometheus, Grafana, and Jaeger locally. The repository also includes separate API, worker, migration, and dashboard container images plus a Docker Compose staging workflow with health checks and automatic rollback.
...
| Operations | Docker, Docker Compose, GitHub Actions, Prometheus, Grafana, OpenTelemetry, Jaeger, Sentry |
...
deploy/                   Local observability and Docker Compose staging assets
docs/                     API contract
...
The local product paths are complete and tested. Services that depend on third-party accounts or credentials remain inactive until configured.
